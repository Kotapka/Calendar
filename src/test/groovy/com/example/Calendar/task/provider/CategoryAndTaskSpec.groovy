...
import com.example.Calendar.InMemoryAssignedTaskRepository
import com.example.Calendar.InMemoryCategoryRepository
import com.example.Calendar.InMemoryCustomerRepository
import com.example.Calendar.InMemoryTaskRepository
...
class CategoryAndTaskSpec extends Specification implements Samples {
    ...
    //3
    def "Should not create category if category with same name for specific user already exist"() {
        given: "User exist"
        customerRepository.save(USER_EXAMPLE)
    ...
    //11
    def "Should not create category without user"() {
        given: "User exist"
        customerRepository.save(USER_EXAMPLE)
        when: "Category without name is created"
        CategoryDto schoolCategoryDto = CategoryDto.builder().name("CategoryName").user("").build()
        categoryOperationFacade.saveCategory(schoolCategoryDto)
        then: "Exception is thrown"
        thrown(HttpException)
    }
